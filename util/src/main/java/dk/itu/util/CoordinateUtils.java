...
public class CoordinateUtils {
    ...
    public static float[] utmToWgs(double lon, double lat) {
        ProjCoordinate result = new ProjCoordinate();
        var transformed = utmToWgs.transform(new ProjCoordinate(lon, lat), result);
        return new float[] {(float) transformed.x, (float) transformed.y};
    }
    public static float[] wgsToUtm(double lon, double lat) {
        ProjCoordinate result = new ProjCoordinate();
