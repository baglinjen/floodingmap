...
public class MapModelOsmFile extends MapModel {
    ...
    private void splitAreaLayers() {
        int topMostLayer = 0;
        Map<Integer, BoundedLayer> levels = new HashMap<>();
        levels.put(topMostLayer, new BoundedLayer(null, new ArrayList<>()));
        ...
        for (OsmElement element : this.areaElements) {
            ...
            if (topLevel.bounds == null) {
                ...
                if (intersects || contains) {
                    ...
                    newLayer.add(way);
                    levels.put(topMostLayer, new BoundedLayer(shapeBounds, newLayer));
                } else {
        ...
        for (BoundedLayer layer : levels.values()) {
